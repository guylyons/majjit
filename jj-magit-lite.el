;;; jj-magit-lite.el --- Magit-inspired UI for Jujutsu -*- lexical-binding: t; -*-
...
;; jj-magit-lite provides a small Magit-inspired status UI and command
...
(defgroup jj-magit-lite nil
  "A small Magit-inspired interface for Jujutsu."
  :group 'tools
  :prefix "jj-magit-")
...
(defun jj-magit--insert-command-section (title root &rest args)
  ...
  (magit-insert-section (jj-command title t)
    ...
    (condition-case err
        (let ((output (apply #'jj-magit--run root args)))
...
(defun jj-magit--render-status (root)
  ...
  (let ((inhibit-read-only t))
    (erase-buffer)
    (insert (propertize "Jujutsu status" 'face 'magit-section-heading) "\n")
...
(defun jj-magit-status-refresh ()
  ...
  (let ((root (jj-magit--default-root)))
    (unless (derived-mode-p 'jj-magit-status-mode)
      (jj-magit-status root))
    (jj-magit--render-status root)
...
(provide 'jj-magit-lite)

;;; jj-magit-lite.el ends here
